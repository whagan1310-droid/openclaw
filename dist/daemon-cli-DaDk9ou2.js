...
import { $t as loadConfig, Cn as secretRefKey, Ft as resolveSecretRefValues, Qt as createConfigIO } from "./model-selection-BaeHlT6A.js";
import { a as hasConfiguredSecretInput, d as resolveSecretInputRef, l as normalizeSecretInputString } from "./types.secrets-CmT3i4wb.js";
...
import { n as pickPrimaryTailnetIPv4 } from "./tailnet-D1FN3bC3.js";
import { d as resolveGatewayBindHost } from "./ws-CtzAmMCA.js";
import { a as inspectPortUsage, c as classifyPortListener, l as formatPortDiagnostics } from "./ports-BOicksP0.js";
import { n as callGateway, p as loadGatewayTlsRuntime } from "./call-CqTAcmXH.js";
import { r as isWSLEnv } from "./wsl-BPV7mhBo.js";
import { t as killProcessTree } from "./kill-tree-CgXplzwx.js";
import { g as resolveControlUiLinks } from "./onboard-helpers-CKPkIyNp.js";
import { t as formatDocsLink } from "./links-B0L7hhCO.js";
import { n as withProgress } from "./progress-D-QR55qI.js";
import { f as resolveGatewaySystemdServiceName, l as resolveGatewayLaunchAgentLabel } from "./constants-WCso00mE.js";
...
import { t as buildGatewayInstallPlan } from "./daemon-install-helpers-DCJKVFCc.js";
...
async function resolveDaemonProbeToken(params) {
	...
	if (authMode === "password" || authMode === "none" || authMode === "trusted-proxy") return;
	if (authMode !== "token") {
		if (trimToUndefined(params.explicitPassword) || readGatewayPasswordEnv(params.mergedDaemonEnv) || (hasConfiguredSecretInput(params.daemonCfg.gateway?.auth?.password, defaults) ? "__configured__" : void 0)) return;
	}
	const token = trimToUndefined((await resolveSecretRefValues([ref], {
		config: params.daemonCfg,
		env: params.mergedDaemonEnv
	})).get(secretRefKey(ref)));
	if (!token) throw new Error("gateway.auth.token resolved to an empty or non-string value.");
	return token;
}
async function resolveDaemonProbePassword(params) {
	...
	if (authMode !== "password") {
		if (trimToUndefined(params.explicitToken) || readGatewayTokenEnv(params.mergedDaemonEnv) || (hasConfiguredSecretInput(params.daemonCfg.gateway?.auth?.token, defaults) ? "__configured__" : void 0)) return;
	}
...
async function gatherDaemonStatus(opts) {
	...
	const daemonProbeToken = opts.probe ? await resolveDaemonProbeToken({
		daemonCfg,
		mergedDaemonEnv,
		explicitToken: opts.rpc.token,
		explicitPassword: opts.rpc.password
