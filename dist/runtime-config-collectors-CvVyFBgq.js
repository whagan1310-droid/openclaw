import { Cn as secretRefKey, Lt as isNonEmptyString, Rt as isRecord, zt as parseDotPath } from "./model-selection-BaeHlT6A.js";
import { a as hasConfiguredSecretInput, d as resolveSecretInputRef, i as coerceSecretRef } from "./types.secrets-CmT3i4wb.js";
...
function evaluateGatewayAuthSurfaceStates(params) {
	...
	const localTokenSurfaceActive = localTokenCanWin && !envToken && (authMode === "token" || authMode === void 0 && !passwordSourceConfigured);
	...
	const authTokenReason = (() => {
		if (!auth) return "gateway.auth is not configured.";
		if (authMode === "token") return envToken ? "gateway token env var is configured." : "gateway.auth.mode is \"token\".";
		if (authMode === "password" || authMode === "none" || authMode === "trusted-proxy") return `gateway.auth.mode is "${authMode}".`;
		if (envToken) return "gateway token env var is configured.";
...
//#endregion
export { setPathCreateStrict as C, getPath as S, resolvePlanTargetAgainstRegistry as _, createResolverContext as a, isExpectedResolvedSecretValue as b, analyzeCommandSecretAssignmentsFromSnapshot as c, discoverConfigSecretTargets as d, discoverConfigSecretTargetsByIds as f, listSecretTargetRegistryEntries as g, listAuthProfileSecretTargetEntries as h, applyResolvedAssignments as i, collectCommandSecretAssignmentsFromSnapshot as l, isKnownSecretTargetType as m, GATEWAY_AUTH_SURFACE_PATHS as n, pushAssignment as o, isKnownSecretTargetId as p, evaluateGatewayAuthSurfaceStates as r, pushWarning as s, collectConfigAssignments as t, discoverAuthProfileSecretTargets as u, assertExpectedResolvedSecretValue as v, setPathExistingStrict as w, deletePathStrict as x, hasConfiguredPlaintextSecretValue as y };