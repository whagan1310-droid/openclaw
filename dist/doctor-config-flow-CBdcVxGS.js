...
import { d as resolveDefaultAgentId } from "./agent-scope-C_RF521q.js";
import { _ as normalizeAccountId, c as normalizeAgentId, g as DEFAULT_ACCOUNT_ID, n as DEFAULT_MAIN_KEY, r as buildAgentMainSessionKey, v as normalizeOptionalAccountId } from "./session-key-CIXZm7v4.js";
import { Cr as resolveCommandResolutionFromArgv, Gr as formatSlackStreamModeMigrationMessage, Jr as resolveDiscordPreviewStreamMode, Kr as formatSlackStreamingBooleanMigrationMessage, Xr as resolveSlackStreamingMode, Xt as migrateLegacyConfig, Yr as resolveSlackNativeStreaming, Zr as resolveTelegramPreviewStreamMode, fr as normalizeTrustedSafeBinDirs, lr as getTrustedSafeBinDirs, sn as OpenClawSchema, tn as readConfigFileSnapshot, ur as isTrustedSafeBinPath } from "./model-selection-BaeHlT6A.js";
import { t as formatCliCommand } from "./command-format-Gp1OUMPH.js";
import { l as normalizeChatChannelId } from "./registry-CEJSZAKm.js";
import { f as parseToolsBySenderTypedKey } from "./dock-BI9_dwQi.js";
import { a as inspectTelegramAccount, c as listTelegramAccountIds, h as formatSetExplicitDefaultToConfiguredInstruction, m as formatSetExplicitDefaultInstruction, p as formatChannelAccountsDefaultPath, u as resolveTelegramAccount, x as listRouteBindings } from "./plugins-DD42H5-v.js";
import { X as canonicalizeMainSessionAlias, l as saveSessionStore } from "./sessions-DL9FheWJ.js";
...
import { t as applyPluginAutoEnable } from "./plugin-auto-enable-jZIG-i4G.js";
import { i as readChannelAllowFromStore } from "./pairing-store-B_ho1sqi.js";
import { t as collectProviderDangerousNameMatchingScopes } from "./dangerous-name-matching-Hv7D3Ghw.js";
import { n as getChannelsCommandSecretTargetIds, s as resolveCommandSecretRefsViaGateway } from "./command-secret-targets-DcsK8ZYD.js";
import { r as shouldMoveSingleAccountChannelKey, t as fetchTelegramChatId } from "./api-gqlIofBq.js";
import { t as note } from "./note-k8AbeMaN.js";
import { t as isWithinDir } from "./path-safety-2FwntvIU.js";
import { n as formatConfigIssueLines } from "./issue-format-DH2qIw2f.js";
import { a as isMattermostMutableAllowEntry, c as normalizeTelegramAllowFromEntry, i as isMSTeamsMutableAllowEntry, n as isGoogleChatMutableAllowEntry, o as isSlackMutableAllowEntry, r as isIrcMutableAllowEntry, s as isNumericTelegramUserId, t as isDiscordMutableAllowEntry } from "./mutable-allowlist-detectors-CuU1D-wd.js";
...
async function maybeRepairTelegramAllowFromUsernames(cfg) {
	...
	const tokens = Array.from(new Set(listTelegramAccountIds(resolvedConfig).map((accountId) => resolveTelegramAccount({
		cfg: resolvedConfig,
		accountId
	})).map((account) => account.tokenSource === "none" ? "" : account.token).map((token) => token.trim()).filter(Boolean)));
	if (tokens.length === 0) return {
		config: cfg,
		changes: [hasConfiguredUnavailableToken ? `- Telegram allowFrom contains @username entries, but configured Telegram bot credentials are unavailable in this command path; cannot auto-resolve (start the gateway or make the secret source available, then rerun doctor --fix).` : `- Telegram allowFrom contains @username entries, but no Telegram bot token is configured; cannot auto-resolve (run onboarding or replace with numeric sender IDs).`]
	};
