import { $ as normalizeAgentId, I as resolvePathViaExistingAncestorSync, J as buildAgentMainSessionKey, K as DEFAULT_AGENT_ID, O as runExec, S as ensureAgentWorkspace, _ as DEFAULT_HEARTBEAT_FILENAME, b as DEFAULT_TOOLS_FILENAME, et as normalizeMainKey, f as resolveSessionAgentId, g as DEFAULT_BOOTSTRAP_FILENAME, h as DEFAULT_AGENT_WORKSPACE_DIR, i as resolveAgentConfig, j as openBoundaryFile, m as DEFAULT_AGENTS_FILENAME, st as normalizeOptionalAccountId, tt as resolveAgentIdFromSessionKey, v as DEFAULT_IDENTITY_FILENAME, x as DEFAULT_USER_FILENAME, y as DEFAULT_SOUL_FILENAME } from "./run-with-concurrency-BdgPUUY-.js";
import { a as resolveGatewayPort, d as resolveRequiredHomeDir, n as STATE_DIR } from "./paths-MKyEVmEb.js";
import { Cr as secretRefKey, Fr as getProcessStartTime, H as parseDurationMs, Ir as isPidAlive, Lr as formatCliCommand, Pr as resolveProcessScopedMap, U as parseByteSize, V as getBlockedNetworkModeReason, Wr as hasConfiguredSecretInput, Yr as resolveSecretInputRef, _t as CHANNEL_IDS, ai as parseBooleanValue, gr as resolveSecretRefValues, n as loadConfig, o as writeConfigFile, t as createConfigIO } from "./config-Bq0-9IRt.js";
import { E as resolveUserPath, N as truncateUtf16Safe, S as normalizeE164, a as createSubsystemLogger, c as defaultRuntime } from "./logger-COmHOvdm.js";
import { J as normalizeHyphenSlug, d as getChannelDock, o as normalizeThinkLevel } from "./thinking-jqu0d5mh.js";
import { T as normalizeMessageChannel, i as getImageMetadata, n as buildImageResizeSideGrid, s as resizeToJpeg, t as IMAGE_REDUCE_QUALITY_STEPS, w as listDeliverableMessageChannels } from "./image-ops-BeAUismH.js";
import { Y as normalizeChatType, r as normalizeChannelId } from "./plugins-NK3Kbpsu.js";
import { a as resolveSessionTranscriptPathInDir, i as resolveSessionTranscriptPath, n as resolveSessionFilePath, s as resolveStorePath, t as resolveDefaultSessionStorePath } from "./paths-DR30B6cs.js";
import { i as writeTextAtomic, r as writeJsonAtomic } from "./json-files-DJe5SBCd.js";
import { t as emitSessionTranscriptUpdate } from "./transcript-events-C2lcxc6-.js";
import { n as extractErrorCode, r as formatErrorMessage } from "./errors-x0EvNKYN.js";
import { t as PATH_ALIAS_POLICIES } from "./path-alias-guards-CtLu-sVZ.js";
import { t as SsrFBlockedError } from "./ssrf-CZCOeNMj.js";
import { t as sanitizeContentBlocksImages } from "./tool-images-ByM5teJW.js";
import { i as resolveWindowsSpawnProgram, n as materializeWindowsSpawnProgram } from "./windows-spawn-E2JqbJ-S.js";
import { a as syncSkillsToWorkspace, h as safeEqualSecret, l as resolveSandboxInputPath, m as sanitizeEnvVars, u as resolveSandboxPath } from "./skills-CSkUhue3.js";
import { A as DEFAULT_DOWNLOAD_DIR, B as DEFAULT_AI_SNAPSHOT_MAX_CHARS, C as ensureChromeExtensionRelayServer, D as PROFILE_POST_RESTART_WS_TIMEOUT_MS, E as PROFILE_ATTACH_RETRY_TIMEOUT_MS, F as resolveWritablePathWithinRoot, G as DEFAULT_OPENCLAW_BROWSER_PROFILE_NAME, H as DEFAULT_BROWSER_EVALUATE_ENABLED, K as isLoopbackHost, L as normalizeBrowserFormField, M as DEFAULT_UPLOAD_DIR, N as resolveExistingPathsWithinRoot, O as resolveCdpReachabilityTimeouts, R as DEFAULT_AI_SNAPSHOT_EFFICIENT_DEPTH, T as CDP_JSON_NEW_TIMEOUT_MS, U as DEFAULT_OPENCLAW_BROWSER_COLOR, V as DEFAULT_BROWSER_DEFAULT_PROFILE_NAME, W as DEFAULT_OPENCLAW_BROWSER_ENABLED, _ as withBrowserNavigationPolicy, a as resolveOpenClawUserDataDir, b as fetchOk, c as resolveBrowserExecutableForPlatform, f as normalizeCdpWsUrl, g as assertBrowserNavigationResultAllowed, h as assertBrowserNavigationAllowed, i as launchOpenClawChrome, j as DEFAULT_TRACE_DIR, l as captureScreenshot, m as InvalidBrowserNavigationUrlError, n as isChromeCdpReady, o as stopOpenClawChrome, p as snapshotAria, r as isChromeReachable, u as createTargetViaCdp, v as appendCdpPath, w as stopChromeExtensionRelayServer, y as fetchJson, z as DEFAULT_AI_SNAPSHOT_EFFICIENT_MAX_CHARS } from "./chrome-cUWm_ik6.js";
import { a as saveMediaBuffer, n as ensureMediaDir } from "./store-B2I8nuEV.js";
import { t as generateSecureToken } from "./secure-random-IkuYAMEf.js";
...
const log$3 = createSubsystemLogger("sessions/store");
...
function pruneStaleEntries(store, overrideMaxAgeMs, opts = {}) {
	...
	if (pruned > 0 && opts.log !== false) log$3.info("pruned stale session entries", {
		pruned,
		maxAgeMs
	});
	return pruned;
...
function capEntryCount(store, overrideMax, opts = {}) {
	...
	if (opts.log !== false) log$3.info("capped session entry count", {
...
async function saveSessionStoreUnlocked(storePath, store, opts) {
	...
	if (!opts?.skipMaintenance) {
		...
		if (shouldWarnOnly) {
			...
			if (activeSessionKey) {
				...
				if (warning) {
					log$2.warn("session maintenance would evict active session; skipping enforcement", {
	...
	if (process.platform === "win32") {
		for (let i = 0; i < 5; i++) try {
			...
			log$2.warn(`atomic write failed after 5 attempts: ${storePath}`);
		}
		return;
	}
	try {
...
function hasGatewayTokenEnvCandidate(env) {
...
async function loadPwAiModule(mode) {
	try {
		return await import("./pw-ai-Bh2o7hAj.js");
	} catch (err) {
...
async function closePlaywrightBrowserConnection() {
	try {
		await (await import("./pw-ai-Bh2o7hAj.js")).closePlaywrightBrowserConnection();
	} catch {}
}
...
function classifyFailoverReasonFromHttpStatus(status, message) {
	...
	if (status === 402) {
		if (message) {
			...
			if ((lower.includes("try again") || lower.includes("retry") || lower.includes("temporary") || lower.includes("cooldown")) && (lower.includes("usage limit") || lower.includes("rate limit") || lower.includes("organization usage"))) return "rate_limit";
		}
		return "billing";
	}
...
function classifyFailoverReason(raw) {
	...
	if (isTransientHttpError(raw)) {
		if (extractLeadingHttpStatus(raw.trim())?.code === 529) return "overloaded";
		return "timeout";
	}
...
//#endregion
export { expandToolGroups as $, canonicalizeMainSessionAlias as $t, isRawApiErrorPayload as A, normalizeAccountId as At, registerBrowserRoutes as B, jsonUtf8Bytes as Bt, isCloudCodeAssistFormatError as C, isCacheEnabled as Ct, isFailoverErrorMessage as D, mergeDeliveryContext as Dt, isFailoverAssistantError as E, deliveryContextKey as Et, isTimeoutErrorMessage as F, parseInlineDirectives as Ft, resolveBrowserControlAuth as G, resolveChannelResetConfig as Gt, resolveProfile as H, resolveSessionLockMaxHoldFromTimeout as Ht, ensureSandboxWorkspaceForSession as I, INPUT_PROVENANCE_KIND_VALUES as It, buildPluginToolGroups as J, resolveThreadFlag as Jt, resolveSandboxConfigForAgent as K, resolveSessionResetPolicy as Kt, resolveSandboxContext as L, applyInputProvenanceToUserMessage as Lt, parseImageDimensionError as M, capArrayByJsonBytes as Mt, parseImageSizeError as N, countToolResults as Nt, isLikelyContextOverflowError as O, normalizeDeliveryContext as Ot, sanitizeUserFacingText as P, extractToolCallNames as Pt, stripPluginOnlyAllowlist as Q, setSessionRuntimeModel as Qt, resolveSandboxRuntimeStatus as R, hasInterSessionUserProvenance as Rt, isBillingAssistantError as S, updateSessionStoreEntry as St, isContextOverflowError as T, deliveryContextFromSession as Tt, getBridgeAuthForPort as U, resolveSessionKey as Ut, resolveBrowserConfig as V, acquireSessionWriteLock as Vt, ensureBrowserControlAuth as W, evaluateSessionFreshness as Wt, expandPolicyWithPluginGroups as X, mergeSessionEntry as Xt, collectExplicitAllowlist as Y, DEFAULT_RESET_TRIGGERS as Yt, mergeAlsoAllowPolicy as Z, resolveFreshSessionTotalTokens as Zt, formatAssistantErrorText as _, loadSessionStore as _t, isMessagingToolDuplicateNormalized as a, ensureSessionHeader as at, getApiErrorPayloadFingerprint as b, updateLastRoute as bt, extractToolCallsFromAssistant as c, resolveBootstrapTotalMaxChars as ct, downgradeOpenAIFunctionCallReasoningPairs as d, trimToUndefined as dt, resolveExplicitAgentSessionKey as en, normalizeToolName as et, downgradeOpenAIReasoningBlocks as f, extractDeliveryInfo as ft, classifyFailoverReasonFromHttpStatus as g, resolveAndPersistSessionFile as gt, classifyFailoverReason as h, resolveMirroredTranscriptText as ht, isMessagingToolDuplicate as i, resolveGroupSessionKey as in, buildBootstrapContextFiles as it, isTransientHttpError as j, archiveSessionTranscripts as jt, isRateLimitAssistantError as k, normalizeSessionDeliveryFields as kt, extractToolResultId as l, sanitizeGoogleTurnOrdering as lt, BILLING_ERROR_USER_MESSAGE as m, appendAssistantMessageToSessionTranscript as mt, validateGeminiTurns as n, deriveSessionMetaPatch as nn, compileGlobPatterns as nt, normalizeTextForComparison as o, resolveBootstrapMaxChars as ot, isGoogleModelApi as p, parseSessionThreadInfo as pt, applyOwnerOnlyToolPolicy as q, resolveSessionResetType as qt, pickFallbackThinkingLevel as r, resolveConversationLabel as rn, matchesAnyGlobPattern as rt, sanitizeSessionMessagesImages as s, resolveBootstrapPromptTruncationWarningMode as st, validateAnthropicTurns as t, resolveMainSessionKey as tn, resolveToolProfilePolicy as tt, sanitizeToolCallIdsForCloudCodeAssist as u, resolveGatewayCredentialsFromConfig as ut, formatBillingErrorMessage as v, readSessionUpdatedAt as vt, isCompactionFailureError as w, resolveCacheTtlMs as wt, isAuthAssistantError as x, updateSessionStore as xt, formatRawAssistantErrorForUi as y, recordSessionMetaFromInbound as yt, createBrowserRouteContext as z, normalizeInputProvenance as zt };