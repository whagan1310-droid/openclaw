...
const ALLOWLIST_SET_CACHE = /* @__PURE__ */ new WeakMap();
...
function resolveAllowlistMatchByCandidates(params) {
	...
	for (const candidate of params.candidates) {
		...
		if (allowSet.has(candidate.value)) return {
			allowed: true,
...
function resolveAllowListSet(allowList) {
	const cached = ALLOWLIST_SET_CACHE.get(allowList);
	if (cached && cached.size === allowList.length) return cached.set;
	const set = new Set(allowList);
	ALLOWLIST_SET_CACHE.set(allowList, {
...
//#endregion
export { DEFAULT_ACCOUNT_ID, GROUP_POLICY_BLOCKED_LABEL, MarkdownConfigSchema, PAIRING_APPROVED_MESSAGE, ToolPolicySchema, addWildcardAllowFrom, applyAccountNameToChannelSection, buildChannelConfigSchema, buildChannelKeyCandidates, buildProbeChannelStatusSummary, createActionGate, createLoggerBackedRuntime, createReplyPrefixOptions, createScopedPairingAccess, createTypingCallbacks, deleteAccountFromConfigSection, emptyPluginConfigSchema, fetchWithSsrFGuard, formatAllowlistMatchMeta, formatDocsLink, formatLocationText, formatPairingApproveHint, formatResolvedUnresolvedNote, hasConfiguredSecretInput, issuePairingChallenge, jsonResult, logInboundDrop, logTypingFailure, mergeAllowFromEntries, mergeAllowlist, normalizeAccountId, normalizeResolvedSecretInputString, normalizeSecretInputString, promptChannelAccessConfig, promptSingleChannelSecretInput, readNumberParam, readReactionParams, readStoreAllowFromForDmPolicy, readStringParam, resolveAllowlistMatchByCandidates, resolveAllowlistProviderRuntimeGroupPolicy, resolveChannelEntryMatch, resolveControlCommandGate, resolveDefaultGroupPolicy, resolveDmGroupAccessWithLists, runPluginCommandWithTimeout, setAccountEnabledInConfigSection, summarizeMapping, toLocationContext, warnMissingProviderGroupPolicyFallbackOnce };