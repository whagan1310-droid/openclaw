...
import { l as normalizeSecretInputString } from "./types.secrets-CmT3i4wb.js";
...
async function runOnboardingWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-CKPkIyNp.js").then((n) => n.d);
	...
	try {
		...
		await prompter.note(["Could not resolve gateway.auth.token SecretRef for onboarding probe.", error instanceof Error ? error.message : String(error)].join("\n"), "Gateway auth");
	}
	...
	try {
		...
		await prompter.note(["Could not resolve gateway.remote.token SecretRef for onboarding probe.", error instanceof Error ? error.message : String(error)].join("\n"), "Gateway auth");
	}
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-DOCoWf0b.js").then((n) => n.t);
		const { logConfigUpdated } = await import("./logging-CEi-PIBm.js").then((n) => n.r);
	...
	const { applyOnboardingLocalWorkspaceConfig } = await import("./onboard-config-LavyKOIh.js").then((n) => n.n);
	...
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-BFv49hiE.js").then((n) => n.t);
	const { promptCustomApiConfig } = await import("./onboard-custom-BgY8axSX.js").then((n) => n.r);
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-CunwcQIy.js").then((n) => n.t);
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-CAeTW1lz.js").then((n) => n.i);
	...
	const { configureGatewayForOnboarding } = await import("./onboarding.gateway-config-Cgn9XxH1.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-DD42H5-v.js").then((n) => n.i);
		const { setupChannels } = await import("./onboard-channels-CI15AJwP.js").then((n) => n.n);
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-DbR2DSEa.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-oZrGmURS.js").then((n) => n.t);
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	const { setupInternalHooks } = await import("./onboard-hooks-BwtViVTJ.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeOnboardingWizard } = await import("./onboarding.finalize-C2-UEeAz.js");
